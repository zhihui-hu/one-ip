...
function loadWidget(id: Provider["id"]): Promise<WidgetApi> {
  ...
  const promise = new Promise<WidgetApi>((resolve, reject) => {
    ...
    function fail() {
      ...
      script.remove();
      scripts.delete(id);
      reject(new Error("验证组件加载失败，请检查网络或内容拦截设置。"));
    }
    script.onerror = fail;
    script.onload = () => {
      const ready = () => {
        ...
        widget = api.render(mount, {
          ...
          callback: async (token: string) => {
            ...
            try {
              ...
              if (!active) return;
              setStatus(result.success ? "验证通过" : "未通过");
              ...
              if (active) {
                setStatus("未完成");
                setMessage(
                  error instanceof Error ? error.message : "验证请求失败",
                );
              }
            }
          },
    ...
    <ToolCard title={provider.name}>
      <div className="row-between gap-3">
        <Badge variant="secondary">
          {provider.configured ? status : "未配置"}
        </Badge>
        <Button
      ...
      <p className="small muted mt-3" role="status">
        {!provider.configured
          ? (provider.reason ?? "当前站点尚未启用此验证。")
          : message || "点击后加载验证服务，按提示完成操作。"}
        {elapsed !== undefined && ` · 用时 ${elapsed} 秒`}
      </p>
    </ToolCard>
  );
}
export default function ChallengesPage() {
  ...
  return (
    <>
      <PageHeading title="验证体验" description="" />
      <ErrorNotice error={query.error} />
      {query.isPending ? (
        <Pending>正在读取验证服务…</Pending>
      ) : (
      ...
      {!!query.data?.length && (
        <div className="mt-3">
          <ToolCard title="结果怎么看？">
            <p className="small muted">
              通过仅表示本站本次验证成功，不代表其他网站也会通过。Turnstile
              体验不等同于 Cloudflare 整站防护挑战。
            </p>
            <p className="small muted mt-2">
              FingerprintJS 用于计算浏览器标识，不提供验证码通过结论。
              <Link to="/browser/fingerprint">查看指纹检测 ›</Link>
            </p>
          </ToolCard>
        </div>
      )}
